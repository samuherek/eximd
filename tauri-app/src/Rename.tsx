...
const renameMachine = setup({
...
}).createMachine(
    {
        initial: 'collecting',
        context: ({ input }) => {
            console.log("context in put", input);
            return {
                items: [],
                source: input.source,
                file_count: null,
        ...
        invoke: {
            src: 'tauriActor',
            onError: { actions: raiseErrorToUI }
        },
        states: {
            collecting: {
                on: {
                    FILES_COLLECTED: {
                        target: "exifing",
                        actions: assign({
                            file_count: ({ event }) => event.payload.file_count,
                            items: ({ event }) => event.payload.files
                        })
                    }
                },
            },
    ...
    return (
        <div className="">
            ...
            <ul>
                {items.map((item: any) => {
                    return (
                        <li key={item.path} className="flex items-center py-2 pl-4 cursor-pointer" onClick={() => send({ type: "SELECT_ITEM", payload: item.paty })}>
                            {item.selected ? (
